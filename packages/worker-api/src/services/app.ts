...
export class AppService {
  ...
  async createApplication(
    ...
    const app = await this.db.createApplication(
      ...
      input.children_policy_url,
      input.terms_of_service_url,
      isOfficial,
      input.redirect_uris,
      input.scopes
    );
    ...
    if (app.validation_status === 'validated') {
      const request = await this.db.createAppChangeRequest({
        ...
        payload: {
          ...
          children_policy_url: input.children_policy_url,
  ...
  async applyApprovedChangeRequest(requestId: string): Promise<ApplicationPublic | null> {
    ...
    const updated = await this.db.updateApplication(
      ...
      typeof payload.privacy_policy_url === 'string' ? payload.privacy_policy_url : undefined,
      typeof payload.children_policy_url === 'string' ? payload.children_policy_url : undefined,
      typeof payload.terms_of_service_url === 'string' ? payload.terms_of_service_url : undefined,
  ...
  private toPublic(app: Application): ApplicationPublic {
    return {
      ...
      children_policy_url: app.children_policy_url,
